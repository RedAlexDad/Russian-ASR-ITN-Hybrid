...
## Анализ ошибок на real.f и synthetic.f

### real.f: 55.23% (95/172)

real.f — реальные тексты Википедии и новостей. Принципиальное отличие от calibration.f:
длинные контексты (средняя длина 123 символа против ~30), пунктуация, смешанный
...
|-----------|--------|---|--------|
| Пунктуация прилипла к токену | 17 | 22% | `девять)`, `пятьдесят%`, `четыре%` |
| Десятки+единицы разбиты пунктуацией | 11 | 14% | `тридцать девять)` → `девять)` не опознан |
| Дефисные конструкции | 9 | 12% | `фаб-пятьсот`, `тридцать-х`, `пять-й` |
| Decimal comma со словом | 7 | 9% | `один,5`, `пять,4`, `три,50` |
| Time-формат (слово:минуты) | 5 | 6% | `двенадцать:19`, `десять:00` |
| Контекстные false positives | ~10 | 13% | `первым`→1, `двух`→2, `трое`→3, `седьмым`→7 |
| Прочее (словоформы, смешанный язык, 41а-семь) | ~18 | 23% | `четверо`, `десяти`, `двенадцать километрах` |

#### Корень проблемы

Токенизатор режет по пробелам, но пунктуация (`)`, `%`, `,`) прилипает к слову.
Классификатор не видит `девять)` как "девять". Аналогично `один,5` — "один" не
связывается с ",5" в единое число.

Контекстные false positives — слова, разделяющие корень с числительными, но
выполняющие другую грамматическую роль: `первый` (порядковое-прилагательное,
не число), `двух` (родительный падеж), `трое` (собирательное).

#### Оценка потолка

После фиксов препроцессинга (отделение пунктуации, decimal/time regex,
стоп-лист контекстных слов) можно поднять до **~85–90%**. Оставшиеся ~10%
требуют pymorphy2-дисамбигуации или T5.

### synthetic.f: 29.48% (4899/16618)

#### Взвешенное среднее

synthetic.f содержит два подмножества с колонкой `noise_level`:

| Подмножество | Строк | Доля | Accuracy |
|---|---|---|---|
| `clean` | 4,890 | 29.4% | **95.87%** |
| `noisy` | 11,728 | 70.6% | **1.80%** |
| **Итого** | **16,618** | **100%** | **29.48%** |

На clean — отличная точность (96%). На noisy — 1.8%, потому что ASR-шум
ломает слова до неузнаваемости:

- `тсыячи` → не `тысячи` (перестановка)
- `чтыреста` → не `четыреста` (замена/пропуск)
- `трста` → не `триста` (пропуск слога)
- `шисдьсат` → не `шестьдесят` (множественные замены)
- `тыcяч` → латиница `c` вместо кириллицы `с`

Rule-based классификатор требует точного совпадения (exact dict) или хотя бы
узнаваемого корня (root matching). ASR-шум ломает и то, и другое. Это
**принципиальное ограничение rule-based подхода** — такие ошибки исправляются
только通过学习 зашумлённых паттернов (T5 / seq2seq).

#### Почему sequence parser не помогает на noisy

Sequence parser улучшил fused compounds (`девятьтысяч` → 9000), но это
помогает только на clean данных. На noisy данные fused compound тоже
зашумлён: `тристатридцать` (clean, работает) vs `тристатридцтаь` (noisy,
не работает).
