...
Формат:  каждая «ячейка» выводит markdown-заголовок → код → результат
"""
...
def number_distribution(cal):
    ...
    plt.close()
    print(f'Всего чисел: {len(all_digits)}, среднее на строку: {len(all_digits)/cal.height:.2f}')

    fig, ax = plt.subplots(figsize=(6, 6))
    has = cal.filter(pl.col('num_count') > 0).height
    no = cal.filter(pl.col('num_count') == 0).height
    ax.pie([has, no], labels=[f'С числами ({has})', f'Без чисел ({no})'],
