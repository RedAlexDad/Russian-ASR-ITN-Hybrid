...
def test_sum_hundreds_tens():
    assert parse_number_group([(200, 3, False, False), (50, 2, False, False)]) == [
        "250"
    ]


def test_enumeration_same_magnitude():
    assert parse_number_group([(200, 3, False, False), (300, 3, False, False)]) == [
        "200",
        "300",
    ]
...
def test_zero_and_seven():
    assert parse_number_group([(0, 0, False, False), (7, 0, False, False)]) == [
        "0",
        "7",
    ]
...
def test_ordinal_in_compound():
    assert parse_number_group(
        [(200, 3, False, False), (80, 1, False, False), (5, 0, False, True)]
    ) == ["285"]
